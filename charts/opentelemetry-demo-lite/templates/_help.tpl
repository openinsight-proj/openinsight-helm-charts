...
{{- $opt := cat  "-Dspring.cloud.nacos.config.enabled=true" " " -}}
...
{{/* metadata k8s */}}
{{- $opt = cat  $opt "-Dspring.cloud.nacos.discovery.metadata.k8s_cluster_id=xxx -Dspring.cloud.nacos.discovery.metadata.k8s_cluster_name=xxx" " " -}}
...
{{- $opt := "-javaagent:./jmx_prometheus_javaagent-0.17.0.jar=12345:./prometheus-jmx-config.yaml " -}}
{{- $opt = cat $opt "-Dotel.metrics.exporter=prometheus" " " -}}
{{- $opt = cat $opt "-Dmeter.port=8888" " " -}}
